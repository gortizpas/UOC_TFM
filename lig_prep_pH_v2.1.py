...
ligand_names = ["NDB", "CDCA", "OCA", "Tropifexor", "TERN-101", "DM175", "Andrographolide"]
ligands = []

# extraccion de smiles
i = 1
for name in ligand_names:    
    ligands.append([open(save + "ligands.smi").read().split()[i-1], name, i, "ligand"])
    i = i + 1


decoys_file = open(save + 'decoys.smi', 'r')
decoys_list = decoys_file.readlines()

decoys = []

i = 1
for line in decoys_list:
    a = [line.split()[0], line.split()[1], i, "decoy"]
    i = i + 1 
    decoys.append(a)
    
all_smiles = ligands + decoys

def smile_to_mol(smile, name, index, mol_type):
    """
    ...
    m.SetProp("_Name", str(smile) + "\t;\t" + str(name))
    ...
    try:
        ...
        opt_fail.append(str("decoy_unopt_" + str(index)))
        print("Optimisation failed before pH evaluation for molecule " + str(index))
    
    # Generates one file per ligand with 3D coordinates. (mol format)
    mols.extend([index, m2, mol_type])
    print(Chem.MolToMolBlock(m2), file=open(save + "original/" + mol_type + "_" + str(index) + ".mol",'w'))

    return mols, opt_fail


def mol_to_smiles(index, mols, mol_type):
    """
    Parameters
    ----------
    index : int
    ...
    smiles = []
    smiles.extend([index, Chem.MolToSmiles(mols, isomericSmiles=True), mol_type])
    
    return smiles

def prot_mol_to_smiles(index, version, mols, mol_type):
    """
    Parameters
    ----------
    index : int
    ...
    smiles = []
    smiles.extend([index, version, Chem.MolToSmiles(mols, isomericSmiles=True), mol_type])
    
    return smiles


def protonation(index, mols, mol_type):
    """
    ...
    for m in protonated_mol:
        ...
        try:
            ...
            opt_pH_fail.append(str("dim_" + mol_type + "_unopt_" + str(index) + "_v" + str(iter)))
            print("Optimisation failed for molecule " + str(index) + "_v" + str(iter))
            
        # Generation of 3D mol file of ligands after pH evaluation
        print(Chem.MolToMolBlock(m), file=open(save + "pH/dim_" + mol_type + "_" + str(index) + "_v" + str(iter) + ".mol",'w'))
        prot_mols.append([index, iter, m, mol_type])
        iter = iter + 1
...
def main(params=None):
    ...
    mols = pool.starmap_async(smile_to_mol, [row for row in all_smiles]).get()
