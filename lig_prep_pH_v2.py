...
Considerations:
    ...
    RDkit must be installed, and this script can't be executed from spyder, it must
    be done from the bash terminal using: 
        python lig_prep_pH_v2.py
        
    It gets a .txt/.smi file with SMILES and prepares molecules from it.
...
save = "/media/gorpas/linux-storage/UOC/docking/docking/FXR/CDCA-decoys/"


# extraccion de smiles
ligand = [open(save + "ligands.smi").read().split()[0], "CDCA", "0"]

decoys_file = open(save + 'decoys.smi', 'r')
decoys_list = decoys_file.readlines()

decoys = []
decoys.append(ligand)

iter = 1
for line in decoys_list:
    a = [line.split()[0], line.split()[1], str(iter)]
    iter = iter + 1 
    decoys.append(a)

def smile_to_mol(smile, name, index):
    """
    ...
    print(Chem.MolToMolBlock(m2), file=open(save + "original/decoy_" + str(index) + ".mol",'w'))
...
def protonation(index, mols):
    ...
    for m in protonated_mol:
        ...
        try:
            ...
            opt_pH_fail.append(str("dim_decoy_unopt_" + str(index) + "_v" + str(iter)))
            print("Optimisation failed for molecule " + str(index) + "_v" + str(iter))
            
        # Generation of 3D mol file of ligands after pH evaluation
        print(Chem.MolToMolBlock(m), file=open(save + "pH/dim_decoy_" + str(index) + "_v" + str(iter) + ".mol",'w'))
...
def main(params=None):
    ...
    mols = pool.starmap_async(smile_to_mol, [row for row in decoys]).get()
