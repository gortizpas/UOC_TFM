...
df4 = pd.read_excel(path + "/" + folders[3] + "/data/" + folders[3] + "_LBECs.xlsx")[["energy", "ligand"]]
df5 = pd.read_excel(path + "/" + folders[4] + "/data/" + folders[4] + "_LBECs.xlsx")[["energy", "ligand"]]
...
df4["energy"] = df4.energy.apply(lambda x: float(x))
df5["energy"] = df5.energy.apply(lambda x: float(x))
...
df4 = df4.groupby(["ligand"]).apply(lambda x: min(x.energy)).reset_index()
df5 = df5.groupby(["ligand"]).apply(lambda x: min(x.energy)).reset_index()


df = pd.merge(df1, df2, on = ["ligand"], how = "outer")
df = pd.merge(df, df3, on = ["ligand"], how = "outer")
df = pd.merge(df, df4, on = ["ligand"], how = "outer")
df = pd.merge(df, df5, on = ["ligand"], how = "outer")
df.columns = ["ligand","en1","en2","en3", "en4", "en5"]

pd.plotting.scatter_matrix(df[["en1","en2","en3", "en4", "en5"]], alpha=0.9, hist_kwds={'bins':300}, s = 0.1)

df["en1m"] = df.en1.apply(lambda x: -1*x)
df["en2m"] = df.en2.apply(lambda x: -1*x) 
df["en3m"] = df.en3.apply(lambda x: -1*x)
df["en4m"] = df.en4.apply(lambda x: -1*x)
df["en5m"] = df.en5.apply(lambda x: -1*x)

df = pareto_rank(df, "ligand", ["en1m","en2m","en3m", "en4m", "en5m"])

df["rankcent"] = df.prank * df.prank.min() / df.prank.max()*100
df.prank.max()
df.prank.min()

df[df.prank <= 20].to_csv(path+"/prueba.csv", sep = ";", index = False)
